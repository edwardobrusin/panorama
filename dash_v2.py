...
st.markdown(f"""
<div class="main-header" style="display: flex; justify-content: space-between; align-items: center; padding: 24px 36px;">
    <div>
        <h1 style="margin: 0; font-size: 2rem;">Panorama Económico</h1>
        <div class="subtitle" style="font-size: 1.15rem; margin-top: 4px;">Principales Variables</div>
    </div>
...
# 3. Generación del bloque HTML dinámico (Rowspans inteligentes)
tbody_html = []
for cat_name, indicadores in secciones_datos:
    ...
    for ind_name, ind_per, variaciones in indicadores:
        ...
        for var_label, var_val, var_tipo, var_bullets in variaciones:
            ...
            # Inserción de filas y mezcla de celdas (rowspan)
            row_html = "<tr>"
            if first_cat:
            ...
            row_html += f"<td style='vertical-align:middle; text-align:left; white-space:nowrap;'><span style='color:#64748B; font-size:0.85rem; display:block; margin-bottom:2px;'>{var_label}</span><span style='{val_css} font-size:1rem;'>{val_str}</span></td>"
            row_html += f"<td style='vertical-align:middle; text-align:left;'>{bull_html}</td>"
            row_html += "</tr>"
            
            tbody_html.append(row_html)

# 4. Renderizado final de la tabla grande
html_gran_tabla = f"""<div class="metric-container card-hover" style="overflow-x:auto; padding: 0;">
<table class="tabla-manual" style="margin: 0; width: 100%;">
<thead>
    <tr>
        <th colspan="2" style="text-align:left; padding-left:14px; width:22%;">Variable Económica</th>
        <th style="text-align:left; width:15%;">Variación<br><span style="font-size:0.75rem; font-weight:normal; text-transform:none; color:#CBD5E1;">(cifras desestacionalizadas)</span></th>
        <th style="text-align:left;">Mensaje relevante</th>
    </tr>
</thead>
<tbody>{''.join(tbody_html)}</tbody>
</table>
</div>"""
